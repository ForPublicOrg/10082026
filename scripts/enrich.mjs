#!/usr/bin/env node
// Metadata enrichment for the redsoil archive: derives the fields that are
...
if (!process.env.REDSOIL_SCRATCH_DIR) {
  console.error(
    "REDSOIL_SCRATCH_DIR is not set. Point it at a directory outside the repo " +
      "for review-file output (e.g. export REDSOIL_SCRATCH_DIR=/tmp/redsoil-scratch).",
  );
  process.exit(1);
}
const SCRATCH_DIR = process.env.REDSOIL_SCRATCH_DIR;
