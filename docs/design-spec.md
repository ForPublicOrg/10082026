# Red Soil — Design Specification

**Domain:** 10082026.com

## 1. What this is

A static archive documenting publicly available videos, photos, and reporting related to a specific protest event — the 2026 Jharkhand student protests against recruitment-exam paper leaks, culminating in the 10 August 2026 march on the Jharkhand Legislative Assembly in Ranchi. Forked from the 20072026.com archive; architecture and editorial rules are unchanged. The objective is **preservation, discoverability, and historical documentation** — not commentary. The site preserves attribution to original sources and avoids editorializing. Verification status is recorded internally per entry (see `verification-policy.md`) but is not displayed anywhere in the UI — nothing in the archive has been independently verified, and the site says so plainly on `/about`.
...
Public "submit a video" form ──────────┼──► src/data/videos.json ─┐
                                        ...
                                        └──────────►  POST/DELETE /api/likes/:id, GET /api/likes/batch
                                            ...
                                            R2 (redsoil-media — public, reference footage)
```
...
- **Cloudflare R2**: `redsoil-media` (public reference footage, served at `media.10082026.com`, not a Worker binding) and `UPLOADS` (private, pending raw uploads from the submit-video form)
...
export const MEDIA_BASE = "https://media.10082026.com/";
```
...
Currently empty — this fork launches before the collect pipeline has run; the feed renders an honest empty state until real footage is collected (see `docs/execution-plan.md` "Open").
...
Every event carries at least one `sources` citation (see `src/data/timeline.json` for the current set, covering the movement from its JSSC-CGL paper-leak origins through the 10 August 2026 march and its aftermath). `sources`/`statements`/`image` are all optional but validated when present: a malformed citation, quote, or image fails the build rather than silently vanishing from the page — an event whose sourcing quietly disappeared is worse than one that never claimed any.

**`MEDIA_BASE`**: single constant in `src/config.ts`, currently `https://media.10082026.com/`. All `media.video`/`media.thumbnail` paths in JSON are relative to it; components join them at render time. Data files never encode an absolute media host.
...
| `/coverage` | Independent-media directory: the coverage that news channels and independent creators (see `src/data/creators.json`) published on their **own** YouTube channels, grouped per channel as horizontal scroll-snap rails of thumbnail cards. Data: `src/data/creators.json`, validated by `loadCreators()` (`src/lib/schema.ts`) at build. **Nothing on this page is re-hosted** — cards link straight to the original uploads (`youtube.com/watch`), thumbnails hotlink `i.ytimg.com` (allowed in the CSP `img-src` for exactly this reason), and each video's confirmed thumb variant + orientation is recorded in data because a static page can't fall back when a variant 404s. Channel avatars are captured once and committed under `public/coverage/` (self-hosted, not hotlinked from `yt3.googleusercontent.com` — those URLs rot when a channel changes its picture, and hotlinking would widen the CSP). Channels with `>= 3` entries get their own rail; the rest pool into one mixed "More from the ground" rail. Desktop ‹ › rail buttons + a sticky channel-chip index with scrollspy are progressive enhancement (`src/scripts/coverage.ts`); without JS the rails are plain scrollable rows. |
| `404` | Real 404 page (no SPA fallback). |

Navigation is a persistent app shell (`src/layouts/Base.astro`), not a page-by-page header: an icon-only bottom tab bar (Home · Feed · Timeline · Coverage · About) on mobile, a fixed left sidebar at `≥768px`, active tab shown by a filled vs. outline glyph plus `aria-current`. `Base.astro` props: `title`, `description`, `ogImage?`, `ogType?`, `canonical?`, `fullBleed?` (removes page padding/document scroll so a page like `/feed` owns its own snap-scrolling), `hideHeader?`, `overlayNav?`, `hideFooter?`. Astro's `<ClientRouter />` morphs between routes instead of a full reload, respecting `prefers-reduced-motion`.
...
- Mode toggle: **link** (a public URL) or **upload** (a raw file, up to 95MB — see the per-file cap below). Optional event date, description, contact; a honeypot field (`website`, off-screen CSS, not `display:none`).
...
- **Per-file cap**: **95MB**, defined once in `src/lib/upload-limits.ts` (`MAX_UPLOAD_BYTES`) and imported by both the Worker and the page — never write the number anywhere else. The ceiling is Cloudflare's *plan-level request-body limit* (Free/Pro 100MB, Business 200MB, Enterprise 500MB), which is enforced at the edge and returns a 413 **before the Worker is invoked**, so a cap above it cannot be caught or explained server-side. This was 250MB until 2026-07-28, when it was found that files between 100MB and 250MB passed every check the site controls, created a `video_submissions` row, and were then killed at the edge — leaving orphaned rows with a NULL `r2_key` while the submitter saw a generic connection error. Raising this requires checking the zone's plan first, or moving to chunked uploads.
- **Upload mode**: two steps. `POST /api/submit-video` (`mode: "upload"`) validates `mimeType`/`fileSize` (the per-file cap above, 5GB/24h rolling cap across all uploads, MIME allowlist mp4/mov/webm/mkv/3gp) and inserts a pending row, returning its `id`. The client then does `PUT /api/upload/:id` with the raw file body via `XMLHttpRequest` (tracking `xhr.upload.progress` for the visible bar); the Worker streams it into R2 bucket `UPLOADS` at key `pending/${id}-${uuid}.${ext}` and updates the D1 row (`r2_key`, `file_size_bytes`, `mime_type`). The endpoint is single-use per id (`r2_key IS NULL` guard prevents replay/overwrite).
- **Error reporting**: the client distinguishes a request that *failed* from one that was *refused*. `XMLHttpRequest`'s `error` event only fires when nothing came back at all; a 4xx/5xx arrives through `load`, so the handler reads the Worker's `{ ok: false, error }` body and shows that, falling back to a status-specific message when the body isn't ours (an edge-generated error page). Before 2026-07-28 both paths rejected with the same opaque `Error` and every failure was shown as "Could not reach the server", which is what hid the 413 above from submitters and maintainer alike.
...
redsoil/
  ...
  src/
    ...
    data/videos.json
    data/timeline.json
    data/creators.json        independent-media directory for /coverage — links out, nothing re-hosted
    lib/schema.ts             types + loadVideos()/loadTimeline()/loadCreators(); imported by pages at build time
    lib/coverage.ts           pure display helpers for /coverage (watch/thumb URLs, duration/date formatting)
    layouts/Base.astro        html shell, meta defaults, header/nav/footer
    components/
      VideoCard.astro
      TimelineEvent.astro
      CreatorSection.astro      one /coverage channel: header + scroll-snap rail
      CoverageVideoCard.astro   one /coverage card: yt thumbnail + duration + title, links to YouTube
      VerificationBadge.astro   on disk, unimported — see §5
    pages/
      index.astro              landing + submit-video form
      feed.astro
      timeline.astro
      coverage.astro           independent-media directory — see §6
      about.astro
      takedown.astro
      404.astro
      video/[id].astro
    scripts/feed.ts            IntersectionObserver + scrubber + caption-expand + reshuffle (client)
    scripts/coverage.ts        /coverage rail ‹ › controls + chip scrollspy (progressive enhancement)
  scripts/
