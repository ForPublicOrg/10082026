...
const ORIGIN = "https://10082026.com";
...
describe("www redirect", () => {
  it("301s www.10082026.com to the bare domain, preserving path and query", async () => {
    const res = await SELF.fetch("https://www.10082026.com/feed?x=1", {
      redirect: "manual",
    });
    expect(res.status).toBe(301);
    expect(res.headers.get("location")).toBe("https://10082026.com/feed?x=1");
  });
});
