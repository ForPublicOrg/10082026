...
// The feed's behavior differs between an empty archive (the launch state of
// this fork, before the collect pipeline has run) and one with videos — read
// the data file once and let each test skip itself when it doesn't apply.
const videosPath = fileURLToPath(new URL("../../src/data/videos.json", import.meta.url));
const HAS_VIDEOS =
  (JSON.parse(readFileSync(videosPath, "utf-8")) as Array<{ sample?: boolean }>).filter(
    (v) => !v.sample,
  ).length > 0;

test("feed page renders video cards", async ({ page }) => {
  test.skip(!HAS_VIDEOS, "archive has no public videos yet");
...
test("feed shows an honest empty state when the archive has no videos", async ({ page }) => {
  test.skip(HAS_VIDEOS, "archive has videos — empty state not reachable");
  await page.goto("/feed");
  await expect(page.locator(".feed-empty")).toBeVisible();
  await expect(page.locator(".feed-empty")).toContainText(/no videos in the archive yet/i);
});
...
test.describe.serial("like feature", () => {
  ...
  // The count shown right after page load comes from GET /api/likes/batch,
  // which is edge-cached for up to 20 seconds (see handleLikesBatch in
  // src/worker.ts). On a small catalog, the shuffled "first card" is often
  // one an earlier test in this serial block already liked under a
  // different client id — so the hydrated number can be stale, and the
  // POST/DELETE response then reconciles the display to the server's real
  // aggregate. Exact-total assertions must therefore compare against the
  // SETTLED post-action value, never against arithmetic on the hydrated
  // one. (The parent archive's exact assertions only survived because with
  // 187 videos two tests almost never landed on the same card.)
  async function settle(likeBtn: Locator): Promise<void> {
    // data-pending is set synchronously in both like paths and removed in
    // their finally blocks — its absence means the network round-trip and
    // the reconciling applyLikeState() are done.
    await expect.poll(async () => await likeBtn.getAttribute("data-pending")).toBe(null);
  }
  ...
  test("double-tapping a card's video likes it", async ({ page }) => {
    ...
    await settle(likeBtn);
    // >= rather than ===: `before` may be a stale cached total (see
    // settle()'s comment); what a like guarantees is that the settled
    // server total includes this client's new like on top of whatever the
    // true prior total was.
    expect(await readCount(count)).toBeGreaterThanOrEqual(before + 1);
  });
  ...
  test("repeating the double-tap on the same card does not like it again", async ({ page }) => {
    ...
    await expect(likeBtn).toHaveAttribute("data-liked", "true");
    await settle(likeBtn);
    const settled = await readCount(count);
    ...
    await expect(likeBtn).toHaveAttribute("data-liked", "true");
    // Past TAP_WINDOW_MS — if the second double-tap had wrongly issued a
    // network call, its re-count would have landed by now.
    await page.waitForTimeout(500);
    await expect(count).toHaveText(String(settled));
  });
  ...
  test("clicking the like button toggles data-liked and moves the count by exactly one", async ({
    ...
    await expect(likeBtn).toHaveAttribute("data-liked", "true");
    await settle(likeBtn);
    const liked = await readCount(count);
    expect(liked).toBeGreaterThanOrEqual(before + 1);

    await likeBtn.click();
    await expect(likeBtn).toHaveAttribute("data-liked", "false");
    await settle(likeBtn);
    // The DELETE removes exactly this client's one row, so the settled
    // totals differ by exactly one — this is the "moves the count by
    // exactly one" guarantee, asserted between two settled server values.
    expect(await readCount(count)).toBe(liked - 1);
  });
  ...
  test("with reduced motion, the like button still works and double-tapping the video does not like it", async ({
    ...
    // Side effect of the two rapid clicks above: with native controls shown
    // (the reduced-motion mode under test), Chrome's OWN dblclick handler
    // fullscreens the video — browser behavior, not site code. Leave
    // fullscreen before exercising the button, or the fullscreen video
    // intercepts every later click in this test. (Keyboard Escape does not
    // exit fullscreen in headless Chromium — call the API directly.)
    await page.evaluate(() =>
