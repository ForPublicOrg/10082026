...
**Fork status:** the D1 `database_id`s in `wrangler.jsonc` are placeholders — the six databases, the two `UPLOADS` buckets, and the public `redsoil-media` bucket do not exist yet. `.claude/skills/deploy/SKILL.md` §2 is the checklist for creating them. Collected media is currently committed under `public/media/` and served site-relative (`MEDIA_BASE = "/media/"`) until the R2 cutover.

**Vercel deployment path:** the site also deploys as a static build on Vercel (repo import, Astro auto-detected, Node 22). There the Worker's `/api/*` routes don't exist; instead `api/submit-video.js` (a Vercel serverless function on the same path) backs the landing page's link submissions by appending each one to a single rolling `incoming-submissions` branch/PR against the master collection CSV — one queue PR, never one PR per submission. It requires a `GITHUB_TOKEN` env var in the Vercel project (fine-grained PAT scoped to only this repo, Contents + Pull requests read/write). **Privacy contract: the `contact` field is dropped on this path and must never reach a public PR; takedown requests must never become PRs at all.** Raw uploads and likes remain Cloudflare-only features.
